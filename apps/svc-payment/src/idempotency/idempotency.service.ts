...
export class IdempotencyService {
  ...
  async checkAndLock(
    ...
    canProcess: boolean;
    reason?: string;
    existingRecord?: IIdempotencyRecord;
  }> {
    ...
    try {
      ...
      if (existingData) {
        const record = JSON.parse(existingData) as IIdempotencyRecord;
  ...
  async markCompleted(
    ...
    const existingData = await this.redis.get(key);
    if (existingData) {
      const existing = JSON.parse(existingData) as IIdempotencyRecord;
      record.createdAt = existing.createdAt;
    }

    await this.redis.setex(key, this.KEY_TTL_SECONDS, JSON.stringify(record));
  ...
  async getRecord(idempotencyKey: string): Promise<IIdempotencyRecord | null> {
    ...
    return data ? (JSON.parse(data) as IIdempotencyRecord) : null;
  }
}
