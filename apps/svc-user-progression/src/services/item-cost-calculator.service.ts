...
export class ItemCostCalculatorService {
  ...
  calculateNextCost(
    ...
    const multiplierBigInt = BigInt(
      Math.floor(multiplierPow * Number(PRECISION)),
    );
  ...
  calculateBulkCost(
    ...
    const base = Number.parseFloat(baseCost);
  ...
  calculateMaxPurchasable(
    ...
    if (budgetNum < a) {
      return { quantity: 0, remaining: budget, totalCost: '0' };
    }
    ...
    if (inner <= 0) {
      ...
      const totalCost = this.calculateBulkCost(
        baseCost,
        amountOwned,
        maxQuantity,
        multiplier,
    ...
    const budgetBigInt = BigInt(budget.split('.')[0] ?? '0'); // Remove decimals
    const remaining = (budgetBigInt - totalCostBigInt).toString();

    return { quantity, remaining, totalCost };
  }
  ...
  getItemCostCalculation(
    ...
    const nextCost = this.calculateNextCost(
      baseCost,
      currentOwned,
      costMultiplier,
    );
  ...
  calculateSellValue(
    ...
    // Apply sell ratio
    const sellValue = Math.floor(Number.parseFloat(purchaseCost) * sellRatio);
  ...
  calculateItemLevelUpCost(
    baseCost: string,
    currentLevel: number,
    levelMultiplier = 2, // Each level costs 2x the previous
  ): string {
    const base = Number.parseFloat(baseCost);
